...
/**
 * Remote Sensing Service — Business Logic Layer
 * Điều phối giữa Controller, Repository và MinIO Storage Service.
 *
 * Luồng upload:
 *   1. Upload file lên MinIO (stream từ buffer của multer)
 *   2. Insert metadata vào PostgreSQL
 *   3. Insert file record
 *   4. Tạo processing job
 *   Rollback: Nếu DB fail → xóa object trên MinIO
 */
...
const minio   = require('./minioStorage.service');
...
/**
 * Upload ảnh GeoTIFF lên MinIO rồi lưu metadata vào PostgreSQL.
 *
 * @param {Object} options
 * @param {Object} options.metadata        — Body đã validate (Joi)
 * @param {Object} options.rasterFile      — multer file object (buffer)
 * @param {Object} [options.thumbnailFile] — multer file object (buffer)
 * @param {Object} [options.metaJsonFile]  — multer file object (buffer)
 * @param {Object} options.user            — req.user
 * @returns {Promise<Object>} — image record + files + job
 */
const uploadImage = async ({ metadata, rasterFile, thumbnailFile, metaJsonFile, user }) => {
    const imageUuid    = uuidv4();
    const uploadedKeys = []; // Track để rollback nếu cần

    // ── 1. Upload file GeoTIFF chính ─────────────────────────────────────────
    ...
    if (thumbnailFile) {
        ...
        try {
            ...
            // Không fail toàn bộ request nếu thumbnail upload lỗi
            console.warn('[RemoteSensing] Upload thumbnail thất bại:', err.message);
            thumbnailObjectKey = null;
        }
    }

    // ── 3. Upload metadata JSON (nếu có) ─────────────────────────────────────
    ...
    // ── 4. Insert vào PostgreSQL (rollback MinIO nếu fail) ───────────────────
    ...
    try {
        ...
        // 4b. Tạo file record — GeoTIFF chính
        ...
        // 4c. Tạo file record — thumbnail (nếu có)
        ...
        // 4d. Tạo file record — metadata JSON (nếu có)
