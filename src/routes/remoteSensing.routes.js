 ...
 *
 * Public (optionalAuth):
 *   GET  /images               — Danh sách ảnh
 *   GET  /images/:id           — Chi tiết ảnh
 *   GET  /images/:id/cog-url   — COG URL cho WebGIS
 *   GET  /layers               — Layers cho OpenLayers/Mapbox
 *   GET  /images/:id/statistics — Thống kê band
 *
 * Authenticated:
 *   POST   /images                  — Upload ảnh + metadata
 *   GET    /upload-url              — Lấy presigned PUT URL (client upload trực tiếp)
 *   PATCH  /images/:id              — Cập nhật metadata
 *   DELETE /images/:id              — Xóa ảnh (soft/hard)
 *   GET    /images/:id/download     — Presigned download URL
 *
 * Admin only:
 *   POST   /images/:id/process      — Trigger job xử lý
 */

const { Router } = require('express');

const ctrl = require('../controllers/remoteSensing.controller');
const {
    verifyToken,
    optionalAuth,
    requirePermission,
