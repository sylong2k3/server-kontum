...
/**
 * Tên cột khóa chính thật của bảng vật lý (tra qua pg_index — KHÔNG giả định
 * là 'id'). Một số layer nạp từ FileGDB/ArcGIS dùng 'OBJECTID' thay vì 'id'
 * do ogr2ogr chỉ ép FID='id' cho import qua chính app này.
 * Fallback về 'id' nếu bảng không có primary key khai báo.
 *
...
/**
 * Whitelist tên cột thuộc tính có thể ghi của bảng vật lý (loại trừ PK và
 * cột geometry). Dùng để chặn field-update ghi vào cột lạ/không tồn tại.
 *
 * @param {object} layer  - row layer_registry (schema_name, table_name, geometry_column)
 * @returns {Promise<string[]>} danh sách tên cột hợp lệ
 */
const getEditableColumns = async (layer, client = null) => {
    const geometryColumn = layer.geometry_column || 'geom';
    const pkColumn = await getPrimaryKeyColumn(layer, client);
    const { rows } = await exec(client)(
...
/**
 * Ghi 1 feature mới vào bảng vật lý của layer (dùng cho mobile field-updates —
 * so_nnmt đo đạc GPS ngoài hiện trường). Chỉ hỗ trợ layer POINT/MULTIPOINT.
 *
 * @param {object} client     - pg client (trong transaction)
 * @param {object} layer      - row layer_registry
 * @param {object} opts       - { lng, lat, attributes: {col: value} }
 * @returns {Promise<object>} { id, geometry (GeoJSON), properties }
 */
const insertFeature = async (client, layer, { lng, lat, attributes = {} }) => {
    const ref  = tableRef(layer);
    const gcol = geomCol(layer);
    const epsg = Number(layer.epsg_code || 4326);
    const pkCol = qid(await getPrimaryKeyColumn(layer, client));

    const cols = Object.keys(attributes).map((c) => qid(c));
    const params = [lng, lat, ...Object.values(attributes)];
    const colPlaceholders = cols.map((_, i) => `$${i + 3}`);

    const sql = `
        INSERT INTO ${ref} (${gcol}${cols.length ? ', ' + cols.join(', ') : ''})
        VALUES (ST_Transform(ST_SetSRID(ST_MakePoint($1, $2), 4326), ${epsg})${cols.length ? ', ' + colPlaceholders.join(', ') : ''})
        RETURNING ${pkCol} AS id, ST_AsGeoJSON(ST_Transform(ST_SetSRID(${gcol}, ${epsg}), 4326), 6)::json AS geometry`;

    const { rows } = await client.query(sql, params);
    return { id: rows[0].id, geometry: rows[0].geometry, properties: attributes };
};

/**
 * Đọc 1 feature theo id (dùng để snapshot old_data trước khi update).
 */
const findFeatureById = async (client, layer, featureId) => {
    const ref  = tableRef(layer);
    const rawGeom = assertIdentifier(layer.geometry_column || 'geom', 'geometry_column');
    const pkCol = qid(await getPrimaryKeyColumn(layer, client));
    const { rows } = await client.query(
        `SELECT to_jsonb(t.*) - '${rawGeom}' AS properties FROM ${ref} t WHERE ${pkCol} = $1`,
        [featureId]
    );
    return rows[0] ? rows[0].properties : null;
};

/**
 * Cập nhật 1 feature đã tồn tại (attributes + tùy chọn tọa độ mới).
 *
 * @returns {Promise<object|null>} { id, geometry } hoặc null nếu không tìm thấy
 */
const updateFeature = async (client, layer, featureId, { lng, lat, attributes = {} }) => {
    const ref  = tableRef(layer);
    const gcol = geomCol(layer);
    const epsg = Number(layer.epsg_code || 4326);
    const pkCol = qid(await getPrimaryKeyColumn(layer, client));
    ...
    const setClauses = [];
    const params = [];
    Object.entries(attributes).forEach(([col, value]) => {
        params.push(value);
        setClauses.push(`${qid(col)} = $${params.length}`);
    });
    if (lng != null && lat != null) {
        params.push(lng, lat);
        setClauses.push(`${gcol} = ST_Transform(ST_SetSRID(ST_MakePoint($${params.length - 1}, $${params.length}), 4326), ${epsg})`);
    }
    if (setClauses.length === 0) {
        const properties = await findFeatureById(client, layer, featureId);
        if (!properties) { return null; }
        const { rows: geomRows } = await client.query(
            `SELECT ST_AsGeoJSON(ST_Transform(ST_SetSRID(${gcol}, ${epsg}), 4326), 6)::json AS geometry FROM ${ref} WHERE ${pkCol} = $1`,
            [featureId]
        );
        return { id: featureId, geometry: geomRows[0]?.geometry || null };
    }

    params.push(featureId);
    const sql = `
        UPDATE ${ref} SET ${setClauses.join(', ')}
        WHERE ${pkCol} = $${params.length}
        RETURNING ${pkCol} AS id, ST_AsGeoJSON(ST_Transform(ST_SetSRID(${gcol}, ${epsg}), 4326), 6)::json AS geometry`;
...
module.exports = {
    ...
    findByTableName, findFeatureById, findFeaturesAsGeoJSON, findImportJobById,
    geometryColumnExists, getEditableColumns, insertEditHistory, insertFeature,
    listImportJobs, markPublished, markUnpublished, physicalTableExists,
    refreshStats, setActive, updateFeature, updateImportJob,
